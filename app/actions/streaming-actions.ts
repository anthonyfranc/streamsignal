...
export async function getStreamingServices(): Promise<StreamingService[]> {
  try {
    ...
    console.error("Unexpected error in getStreamingServices:", error)
    return []
  }
}
...
export async function getChannels(): Promise<Channel[]> {
  try {
    const supabase = await createClient()
    const { data, error } = await supabase.from("channels").select("*").order("popularity", { ascending: false })
...
export async function getServiceChannels(): Promise<ServiceChannel[]> {
  try {
    const supabase = await createClient()
    const { data, error } = await supabase.from("service_channels").select("*")
    ...
    console.error("Unexpected error in getServiceChannels:", error)
    return []
  }
}
...
export async function getServicesWithChannelCount(): Promise<StreamingService[]> {
  try {
    ...
    const { data: channelCounts, error } = await supabase
    ...
    console.error("Unexpected error in getServicesWithChannelCount:", error)
    return []
  }
}
...
export async function getServiceChannelsById(serviceId: number): Promise<{ channels: Channel[] }> {
  try {
    ...
    console.error("Unexpected error in getServiceChannelsById:", error)
    return { channels: [] }
  }
}
...
export async function getRelatedServices(serviceId: number): Promise<StreamingService[]> {
  try {
    ...
    if (channels.length === 0) {
      ...
      const { data, error } = await supabase.from("streaming_services").select("*").neq("id", serviceId).limit(3)
    ...
    const { data: serviceChannels, error } = await supabase
      .from("service_channels")
    ...
    console.error("Unexpected error in getRelatedServices:", error)
    return []
  }
}
