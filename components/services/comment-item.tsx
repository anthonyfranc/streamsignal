...
interface CommentItemProps {
  ...
  serviceId: number
  onReplySubmitted?: (parentId: number, reply: ReviewComment) => void
}

function CommentItemComponent({ comment, serviceId, onReplySubmitted }: CommentItemProps) {
  ...
  const handleReplySubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (result.success) {
          ...
          // If a reply was submitted successfully and the parent has the callback
          if (result.data && onReplySubmitted) {
            onReplySubmitted(comment.id, result.data)
          }
        }
      } catch (error) {
    ...
    [replyContent, currentUser, comment?.id, comment?.nesting_level, serviceId, submitReply, onReplySubmitted],
  )
  ...
  const handleReaction = useCallback(
    async (reactionType: "like" | "dislike") => {
      ...
      reactToComment(safeNumber(comment?.id, 0), reactionType)
    },
