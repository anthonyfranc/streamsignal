...
const generateChannelName = (table: string, filter: Filter | null) => {
...
export function useSupabaseRealtime<T>(
  ...
  // Generate a stable channel name based on table and filter
  const baseChannelName = generateChannelName(table, filter)

  // Store the channel name in a ref to keep it stable across renders
  const channelNameRef = useRef<string>(baseChannelName)
  ...
  // Track if we've initialized the channel
  ...
  useEffect(() => {
    ...
    visibilityTimerRef.current = setTimeout(() => {
      ...
      if (!isInitializedRef.current || lastVisibleRef.current !== isVisible) {
        ...
        // If we already have a channel, don't recreate it
        ...
        // Create a unique channel name by adding a timestamp
        const channelName = `${channelNameRef.current}_${Date.now()}`
    ...
    }, 300) // Debounce visibility changes by 300ms
